...
while not WindowShouldClose():
    ...
    DrawLine camera.target.x.int, -screenHeight * 10, camera.target.x.int, screenHeight * 10, GREEN
    DrawLine -screenWidth * 10, camera.target.y.int, screenWidth * 10, camera.target.y.int, GREEN
    EndMode2D()
